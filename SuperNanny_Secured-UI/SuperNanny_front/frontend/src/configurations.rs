...
pub fn configurations() -> Html {
    ...
    let selected_role = use_state(|| None as Option<i32>);
    ...
    // ------------------------------------------------------------------
    {
        ...
        spawn_local(async move {
            ...
            let resp = Request::get("http://127.0.0.1:8081/check_auth")
                ...
                .await;
            match resp {
                Ok(r) if r.status() == 200 => auth_status_handle.set(AuthStatus::Valid),
                _ => auth_status_handle.set(AuthStatus::Invalid),
            }
        });
    }
        ...
        envs.iter()
            .filter(|policy| {
                if let Some(rid) = sr {
                    ...
                    // Pas de rôle sélectionné => on n'affiche rien
                    // (ou vous pouvez renvoyer true si vous voulez tout afficher)
                    false
                }
            })
            .cloned()
        ...
        use_effect_with_deps(
            {
                ...
                move |sel_env_handle: &UseStateHandle<Option<AppPolicy>>| {
                    // Pour accéder à la valeur interne:
    ...
    let on_role_change = {
        ...
        Callback::from(move |e: Event| {
            if let Some(select) = e.target_dyn_into::<web_sys::HtmlSelectElement>() {
                ...
                // Tenter de parser en i32
                if let Ok(id) = val.parse::<i32>() {
                    selected_role_state.set(Some(id));
                } else {
                    selected_role_state.set(None);
                }
            }
        })
    };
    ...
    // ------------------------------------------------------------------
    // 9) Supprimer la config sélectionnée
    // ------------------------------------------------------------------
    let on_delete_env = {
        let selected_env_state = selected_env.clone();
        let envs_state = envs.clone();
                    ...
                    <div class="columns">
                        ...
                        <div class="column" style="border:1px solid #ccc; padding:1rem;">
                            ...
                            <div class="form-group" style="margin-bottom:1rem;">
                                ...
                                <select onchange={on_role_change.clone()}>
                        ...
                        <div class="column" style="border:1px solid #ccc; padding:1rem; margin-left:1rem;">
                            ...
                            <div style="margin-top:2rem;">
                                <div class="form-group">
                                    <label>{ "Nom du programme (app_name)" }</label>
                                    <input
                                        type="text"
                                ...
                                <div class="form-group" style="margin-top:0.5rem;">
                                    <label>{ "tcp_connect (ex: 80:443)" }</label>
                                    <input
                                        type="text"
