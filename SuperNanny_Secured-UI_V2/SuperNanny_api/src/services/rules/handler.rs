...
async fn get_default(state: web::Data<AppState>, rid: web::Path<i32>) -> HttpResponse {
    match db::get_default(&state.db, rid.into_inner()) {
...
struct DefaultBody {
    role_id: i32, default_ro: String, default_rw: String,
    tcp_bind: String, tcp_connect: String,
    allowed_ips: String, allowed_domains: String,
}

#[post("/default_policies")]
async fn create_default(state: web::Data<AppState>, body: web::Json<DefaultBody>) -> HttpResponse {
    let p = db::NewDefaultPolicy {
    ...
    match db::insert_default(&state.db, p) {
...
#[derive(serde::Deserialize)]
struct DefaultPatch {
    default_ro: Option<String>, default_rw: Option<String>,
    tcp_bind: Option<String>, tcp_connect: Option<String>,
    allowed_ips: Option<String>, allowed_domains: Option<String>,
}

#[put("/default_policies/{rid}")]
async fn update_default(state: web::Data<AppState>, rid: web::Path<i32>, body: web::Json<DefaultPatch>) -> HttpResponse {
    let ch = db::DefaultPolicyChangeset {
        default_ro:      body.default_ro.as_deref(),
        default_rw:      body.default_rw.as_deref(),
        tcp_bind:        body.tcp_bind.as_deref(),
        tcp_connect:     body.tcp_connect.as_deref(),
        allowed_ips:     body.allowed_ips.as_deref(),
        allowed_domains: body.allowed_domains.as_deref(),
    };
    match db::update_default(&state.db, rid.into_inner(), ch) {
        Ok(_)  => HttpResponse::Ok().finish(),
        Err(e) => HttpResponse::InternalServerError().body(e.to_string()),
    }
}
...
#[derive(serde::Deserialize)]
struct EnvPatch {
    ll_fs_ro: Vec<String>, ll_fs_rw: Vec<String>,
    ll_tcp_bind: Option<String>, ll_tcp_connect: Option<String>,
    allowed_ips: Option<String>, allowed_domains: Option<String>,
}
